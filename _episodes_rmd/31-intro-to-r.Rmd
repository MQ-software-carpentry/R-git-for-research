...
assignment operator `<-`, and the value we want to give it. Type the following into your `Introduction.R` script file and run it. Throughout this lesson, we will be saving our code to script files and tracking our changes using the `Git` tab.
...
At this stage we want to make sure we have saved our script and tracked changes. Save your script file if you have not done so already. Navigate to the `Git` tab in RStudio. As we have done previously, tick the box next to the file you want to stage (in this case, Introduction.R), and click `Commit`. Remember that you will need a mandatory commit message. For example, in this case we could type, "Add code to demonstrate object assignment and comments in R." Click `Commit`. To send your changes to the remote repository on GitHub, click `Push`. 
...
Let's save, commit, and push our script again. You should be starting to feel comfortable with this process. Remember that your changes will not be committed or pushed unless they are saved first! 
...
Track your changes (Save, Commit, and Push)!
...
Let's do a final commit and push for this lesson. When the commit window opens, take a look at the bottom part of the window. You should see a copy of the script you are committing with some green (and possibly some pink) highlights. This shows the changes or "differences", also referred to as "diffs", you have made. Additions are highlighted in green and deletions in pink. You can also view the diffs between your current version and your last commit, by using the `Diff` button directly from the `Git` tab. 
